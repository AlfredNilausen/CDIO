...
class DetectorConfig:
    ...
    # hue støtte
    orange_hue_ratio_min: float = 0.10
    orange_hue_ratio_strict: float = 0.18
...
def normalize_channel_local(channel: np.ndarray, blur_k: int) -> np.ndarray:
    """
    Normaliserer lys/skygge lokalt:
    lyse objekter på mørk baggrund og omvendt bliver mere sammenlignelige.
    """
    k = ensure_odd(blur_k)
    bg = cv2.GaussianBlur(channel, (k, k), 0).astype(np.float32)
    ch = channel.astype(np.float32)

    norm = ch * 128.0 / (bg + 1.0)
...
def compute_white_mask(frame: np.ndarray, cfg: DetectorConfig) -> np.ndarray:
    ...
    H, S, V = cv2.split(hsv)
    Vn = normalize_channel_local(V, cfg.illum_blur_k)
    ...
    mask_norm_bright = Vn > cfg.white_norm_v_min
    ...
    white_mask = (
        mask_low_sat &
        (mask_local_gray | mask_local_v | mask_norm_bright) &
        (~orange_like)
    ).astype(np.uint8) * 255
...
def compute_orange_mask(frame: np.ndarray, cfg: DetectorConfig) -> np.ndarray:
    ...
    # Orange skal have:
    # - korrekt hue
    # - relativt høj saturation
    # - rimelig brightness
    # - rimelig normalized brightness (for at undertrykke skygger)
    mask_h = (H >= cfg.orange_h_min) & (H <= cfg.orange_h_max)
    mask_s = S >= cfg.orange_s_min
    mask_v = V >= cfg.orange_v_min
    mask_vn = Vn >= cfg.orange_norm_v_min
...
def circle_norm_v(frame, cx: int, cy: int, r: int, ratio: float, blur_k: int) -> float:
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    _, _, V = cv2.split(hsv)
    Vn = normalize_channel_local(V, blur_k)
    roi, mask = get_circle_roi(Vn, cx, cy, r, ratio)
    values = roi[mask > 0]
    return float(np.mean(values)) if len(values) > 0 else 0.0
...
def circle_hue_ratio(frame, cx: int, cy: int, r: int, ratio: float, cfg: DetectorConfig) -> float:
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    H, S, V = cv2.split(hsv)
    Vn = normalize_channel_local(V, cfg.illum_blur_k)

    roi_h, mask = get_circle_roi(H, cx, cy, r, ratio)
    roi_s, _ = get_circle_roi(S, cx, cy, r, ratio)
    roi_vn, _ = get_circle_roi(Vn, cx, cy, r, ratio)
    ...
    orange_pixels = (
        (roi_h >= cfg.orange_h_min) &
        (roi_h <= cfg.orange_h_max) &
        (roi_s >= cfg.orange_s_min) &
        (roi_vn >= cfg.orange_norm_v_min) &
        valid
    )
    total = np.count_nonzero(valid)
    pos = np.count_nonzero(orange_pixels)
...
def classify_ball(frame, white_mask, orange_mask, cx: int, cy: int, r: int, cfg: DetectorConfig):
    ...
    norm_v = circle_norm_v(frame, cx, cy, r, cfg.inner_radius_ratio, cfg.illum_blur_k)
    ...
    orange_hue_ratio = circle_hue_ratio(frame, cx, cy, r, cfg.orange_inner_ratio, cfg)

    # kombiner de to orange-signaler
    orange_signal = max(orange_ratio, orange_hue_ratio)
    ...
    if (
        ...
        and norm_v > cfg.white_norm_v_min - 6
        and white_ratio > cfg.white_ratio_threshold
        and white_ratio >= orange_signal
    ):
        white_ok = True
    ...
    orange_ok = False
    if (
        orange_signal > cfg.orange_strict_ratio_threshold
        and saturation > 95
    ):
        orange_ok = True

    if (
        orange_signal > cfg.orange_ratio_threshold
        and saturation > cfg.orange_saturation_min
        and orange_signal >= white_ratio
    ):
        orange_ok = True

    if (
        orange_hue_ratio > cfg.orange_hue_ratio_strict
        and saturation > 90
    ):
        orange_ok = True

    # FINAL DECISION
    if orange_ok and (orange_signal > white_ratio or saturation > 140):
        label = "ORANGE"
    elif white_ok and white_ratio >= orange_signal:
        label = "WHITE"
    elif orange_ok:
        label = "ORANGE"
    elif white_ok:
    ...
    features = {
        ...
        "orange_hue_ratio": orange_hue_ratio,
        "orange_signal": orange_signal,
    ...
    if circles is not None:
        ...
        for (cx, cy, r) in kept:
            ...
            print(
                ...
                f"OH={feat['orange_hue_ratio']:.2f} "
                f"OS={feat['orange_signal']:.2f} "
...
def draw_detections(frame, white_balls, orange_balls, unknown_balls):
    ...
    for (cx, cy, r, feat) in white_balls:
        ...
        txt = (
            f"W S{feat['saturation']:.0f} "
            f"Vn{feat['norm_v']:.0f} "
            f"W{feat['white_ratio']:.2f}"
        )
        cv2.putText(dbg, txt, (cx + 6, cy - 6), cv2.FONT_HERSHEY_SIMPLEX,
                    0.40, (0, 255, 255), 1)
    ...
    for (cx, cy, r, feat) in unknown_balls:
        ...
        cv2.putText(dbg, "?", (cx + 6, cy - 6), cv2.FONT_HERSHEY_SIMPLEX,
                    0.50, (0, 0, 255), 1)

    cv2.putText(dbg, f"White balls: {len(white_balls)}", (20, 35),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
    cv2.putText(dbg, f"Orange balls: {len(orange_balls)}", (20, 65),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 128, 255), 2)
    cv2.putText(dbg, f"Unknown: {len(unknown_balls)}", (20, 95),
