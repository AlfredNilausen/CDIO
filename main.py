    ...
    while True:
        ...
        if math.hypot(dx, dy) >= POSITION_TOL_MM:
            ...
            if wp_type == BALL:
                ...
                if total_dist > BALL_SLOWDOWN_DIST_MM + tol:
                    ux, uy = dx / total_dist, dy / total_dist
                    fast_target = (wp_pos[0] - ux * BALL_SLOWDOWN_DIST_MM,
                                   wp_pos[1] - uy * BALL_SLOWDOWN_DIST_MM)
                    print("[robot] Fast approach -> ({:.0f},{:.0f})".format(*fast_target))
                    robot.drive_to_position(fast_target, lambda: start_mm, reverse=(drive_sign < 0),
                                            tol_mm=60, speed=BALL_SLOW_SPEED, timeout=2.0,
                    ...
                    if cur_h2 is not None:
                        robot.turn_to_heading(target_h2, lambda: last_dir_info.get("heading"),
                                              pulse_ms=400, timeout=TURN_TIMEOUT_S,
                                              stop_fn=lambda: robot_stop_req.is_set())
                    if robot_stop_req.is_set():
                        robot_running.clear(); robot_stop_req.clear(); continue

                print("[robot] Slow final approach -> ({:.0f},{:.0f})".format(*wp_pos))
                robot.drive_to_position(wp_pos, lambda: start_mm, reverse=(drive_sign < 0),
                                        tol_mm=tol, speed=BALL_FAST_SPEED, timeout=2.0,
                                        stop_fn=lambda: robot_stop_req.is_set(),
                                        get_heading_fn=lambda: last_dir_info.get("heading"))
            else:
                speed = 20
